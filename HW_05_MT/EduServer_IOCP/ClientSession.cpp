...
void ClientSession::AcceptCompletion()
{
	...
	//한 번 들어온 녀석이 다시 들어오는 것을 방지하기 위한 것
	//처음 들어올 때 mConnected를 1로 변경
	//이후로 들어오면 이미 1이니깐 Crash
	...
	do 
	{
		...
		//while false? 이건 무조건 한 번만 실행하는 건데 이렇게 해야할 이유가 있나?
		//있다! switch case 문 변형이라고 보면
		//어디서 에러가 나든지 후속을 더이상 실행하지 않고 바로 하단 에러 처리로 갈 수 있음
